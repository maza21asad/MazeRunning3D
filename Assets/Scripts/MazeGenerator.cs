...
public class MazeGenerator : MonoBehaviour
{
    ...
    MazeCell[,] maze;                           // An array of maze cells respresenting the maze grid.
    ...
    List<Direction> GetRandomDirections ()
    {
        ...
        while (dir.Count > 0)                       // Loop until our rndDir list is empty.
        {
            ...
            dir.RemoveAt(rnd);                      // Remove that direction so we cannot choose it again.
        }
    ...
    Vector2Int CheckNeighbours()
    {
        ...
        for (int i = 0; i < rndDir.Count; i++)
        {
            ...
            // If the neighbour we just tried to valid, we can return that neighbour. If not, we go again.
            if (IsCellValid(neighbour.x, neighbour.y)) return neighbour;
        }

        // If we tried all directions and didn't find any valid neighbour, we retuen the currentCell values.
        return currentCell;
    }
    ...
    void BreakWalls (Vector2Int primaryCell, Vector2Int secondaryCell)
    {
        ...
        if (primaryCell.x > secondaryCell.x)            // PrimaryCell's Left Wall.
        {
            maze[primaryCell.x, primaryCell.y].leftWall = false;
        } 
        else if (primaryCell.x < secondaryCell.x)       // SecondaryCell's Left Wall.
        {
            maze[secondaryCell.x, secondaryCell.y].leftWall = false;
        }
        else if (primaryCell.y < secondaryCell.y)       // PrimaryCell's Top Wall.
        {
            maze[primaryCell.x, primaryCell.y].topWall = false;
        }
        else if (primaryCell.y > secondaryCell.y)      // SecondaryCell's Top Wall.
        {
    ...
    // (a dead end is a cell with no valid neighbour).
    void CarvePath (int x, int y)
    {
        ...
        while (!deadEnd)
        {
            ...
            else
            {
                ...
                currentCell = nextCell;                                 // Set the current cell to valid neighbour we found.
